...
function compass_logo() {
    $info = compass_get_logo_information();
    ...
    if ( get_theme_mod( 'logo-retina-ready', compass_get_default( 'logo-retina-ready' ) ) ) {
        $width /= 2;
        $height /= 2;
    }
 ...
 */
function compass_has_logo() {
    $information = compass_get_logo_information();
 ...
 */
function compass_get_logo_information( $force = false ) {
    $logo_information = array();

    $logo_information['image'] = get_theme_mod( 'logo', compass_get_default( 'logo' ) );

    if ( ! empty( $logo_information['image'] ) ) {
        $dimensions = compass_get_logo_dimensions( $logo_information['image'], $force );
...
function compass_get_logo_dimensions( $url, $force = false ) {
    // Build the cache key
    $key = 'compass-' . md5( 'logo-dimensions-' . $url . WPZOOM::$themeVersion );
    ...
    if ( false === $dimensions || is_preview() || true === $force ) {
        ...
        $attachment_id = compass_get_attachment_id_from_url( $url );
 ...
 */
function compass_get_attachment_id_from_url( $url = '' ) {
 ...
 * @return void
 */
function compass_display_favicons() {
    $logo_favicon = get_theme_mod( 'logo-favicon', compass_get_default( 'logo-favicon' ) );
    if ( ! empty( $logo_favicon ) ) : ?>
        <link rel="icon" href="<?php echo esc_url( $logo_favicon ); ?>"/>
    <?php endif;
}

add_action( 'wp_head', 'compass_display_favicons' );
